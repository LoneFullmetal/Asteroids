...
class Asteroid:
    ...
    def __init__(self, position):
        ...
        self.velocity = [
            Asteroid.speed*random.uniform(-2.0,7.0),
            Asteroid.speed*random.uniform(-2.0,7.0)
        ]

        self.angle = random.uniform(0.0,0)
        self.spin = random.uniform(0,0)
    ...
    def draw(self, surface):
        ...
        for p in self.real_points:
            if p[0] + p[1] < self.lowpoint[0] + self.lowpoint[1]:
                self.lowpoint = p
        surface.blit(ast, (self.lowpoint[0] - 16, self.lowpoint[1] - 16))
